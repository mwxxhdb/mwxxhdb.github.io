...
# 关于 fountustic-css 项目

- 一个专门为后端开发准备的组件库，旨在提供一套简洁、易用的样式解决方案。它包含了常用的布局、组件和工具类，帮助开发者快速构建现代化的 Web 应用。
- 既能得到和基于框架的组件库一样简洁的写法，又无需花费时间去学习前端框架的不同语法糖以及 SSR、CSR 等概念。
- 不考虑浏览器兼容性，会使用最新的 CSS 特性和语法，适合现代浏览器环境。
